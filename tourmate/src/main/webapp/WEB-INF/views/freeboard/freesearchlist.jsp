...
<body>
			...
			<div>
				...
				<table class="type09">
					...
					<c:forEach items="${free_list}" var="dto">
						<tbody>
							<tr>
								<td align="center">${dto.free_num}</td>
					...
					<c:if test="${dto.mem_grade>2500}">
						<img src="/resources/grade/grade5.jpg" >
								...
								<td align="center"><a
									href="freecontentview?free_num=${dto.free_num}">${dto.free_title}&nbsp;
										(${dto.free_replynum})</a></td>
								<td align="center">${dto.free_date}</td>
								<td align="center">${dto.free_readcount}</td>
							</tr>
						</tbody>
					</c:forEach>
				</table>
				...
				<c:if test="${ count > 0 }">
					...
					<c:if test="${startPage >5 }">
						<a href="freesearchlist?pageNum=${ startPage-5  }&free_title=${free_title}&free_content=${free_content}&mem_id=${mem_id}">[이전] </a>
					</c:if>

					<c:forEach var="i" begin="${startPage }" end="${ endPage }">
						<a href="freesearchlist?pageNum=${i}&free_title=${free_title}&free_content=${free_content}&mem_id=${mem_id}">[${ i }] </a>
					</c:forEach>

					<c:if test="${ endPage < pageCount }">
						<a href="freesearchlist?pageNum=${ startPage+5 }&free_title=${free_title}&free_content=${free_content}&mem_id=${mem_id}">[다음] </a>
					</c:if>
				</c:if>
    ...
    <table>
      ...
      <td>
         ...
         <td colspan="5">
            <form action="freesearchlist" name="search" onSubmit="return openSearch()">
            ...
            <input class="button" value="검색" type="submit" value="검색" />
			</form>
		</td>
      </tr>     
	</table>
