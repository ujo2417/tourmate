		...
		<div class="container">
			...
			<div>
				...
				<table class="type09">
					...
					<c:if test="${dto.mem_grade <= 100 && dto.mem_grade>=0}">
				...
				<c:if test="${ count > 0 }">
					...
					<c:if test="${startPage >5 }">
						<a href="freelist?pageNum=${ startPage-5  }">[이전] </a>
					</c:if>

					<c:forEach var="i" begin="${startPage }" end="${ endPage }">
						<a href="freelist?pageNum=${i}">[${ i }] </a>
					</c:forEach>

					<c:if test="${ endPage < pageCount }">
						<a href="freelist?pageNum=${ startPage+5 }">[다음] </a>
					</c:if>
				</c:if>
