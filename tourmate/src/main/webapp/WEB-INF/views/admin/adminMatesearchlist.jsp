...
<c:if test="${ count > 0 }">
		...
		<c:if test="${startPage >5 }">
			<a href="adminmatelist?pageNum=${ startPage-5  }&mate_title=${mate_title}">[이전] </a>
		</c:if>

		<c:forEach var="i" begin="${startPage }" end="${ endPage }">
			<a href="adminmatelist?pageNum=${i}&mate_title=${mate_title}">[${ i }] </a>
		</c:forEach>

		<c:if test="${ endPage < pageCount }">
			<a href="adminmatelist?pageNum=${ startPage+5 }&mate_title=${mate_title}">[다음] </a>
		</c:if>
	</c:if>
    ...
    <table>
      ...
      <td>
         ...
         <td colspan="5">
            <form action="adminmatesearchlist" name="search" id="adminSearch">
	...
	<!-- <script type="text/javascript">
	$(function () {
		$("#hrid2").attr("type" , "hidden");
		$("#hrid3").attr("type" , "hidden");
	});
	 $('.searchSelect').change(function () {
		if ($('select option:selected').val() == "writer") {
			$("#hrid1").attr("type" , "text");
			$("#hrid2").attr("type" , "hidden").remove();
			$("#hrid3").attr("type" , "hidden").remove();
		}
		if ($('select option:selected').val() == "title") {
			$("#hrid2").attr("type" , "text");
			$("#hrid1").attr("type" , "hidden").remove();
			$("#hrid3").attr("type" , "hidden").remove();
		}
		if ($('select option:selected').val() == "contents") {
			$("#hrid3").attr("type" , "text");
			$("#hrid1").attr("type" , "hidden").remove();
			$("#hrid2").attr("type" , "hidden").remove();
